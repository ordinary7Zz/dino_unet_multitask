...
def _expected_calibration_error_binary(y_true, y_prob, n_bins: int = 10) -> float:
    """
    Expected Calibration Error (ECE) for binary classification.
    Binning is done on the predicted probability of the positive class.
    """
    ...
    for i in range(n_bins):
        ...
        # Include left edge for the first bin, right edge for the last bin
        if i == 0:
...
def _expected_calibration_error_multiclass(probs, labels, n_bins: int = 10) -> float:
    """
    Expected Calibration Error (ECE) for multiclass classification.
    Binning is done on the confidence of the predicted class.
    """
    probs = np.asarray(probs, dtype=np.float32)
    labels = np.asarray(labels, dtype=np.int32).reshape(-1)

    if probs.ndim != 2 or labels.size == 0 or probs.shape[0] != labels.size:
        return 0.0

    # Ignore invalid labels if any
    valid = (labels != -1)
    if not np.any(valid):
        return 0.0
    ...
    for i in range(n_bins):
        ...
        avg_conf = float(conf_bin.mean())
        avg_acc = float(corr_bin.mean())
        weight = conf_bin.size / n
        ece += weight * abs(avg_acc - avg_conf)
...
def classification_bootstrap_metrics(y_probs, y_labels, threshold=0.5, n_boot=2000, ci=0.95, seed=0):
    ...
    指标: accuracy, precision, recall, f1, auroc, auprc, sensitivity, specificity, youden, ece
    """
    ...
    if y_labels.size == 0:
        zero_ci = (0.0, (0.0, 0.0))
        return {k: zero_ci for k in ['accuracy', 'precision', 'recall', 'f1', 'auroc', 'auprc', 'sensitivity', 'specificity', 'youden', 'ece']}
    ...
    metrics_samples = {
        ...
        'auroc': [], 'auprc': [], 'sensitivity': [], 'specificity': [], 'youden': [], 'ece': []
    }
    ...
    for _ in range(n_boot):
        ...
        # ECE for this bootstrap sample
        ece_val = _expected_calibration_error_binary(labels_s, probs_s)
        metrics_samples['ece'].append(float(ece_val))
...
def multiclass_bootstrap_metrics(probs_list, labels_list, n_boot=2000, ci=0.95, seed=0):
    ...
    if labels.size == 0:
        zero_ci = (0.0, (0.0, 0.0))
        return {k: zero_ci for k in ['accuracy', 'precision', 'recall', 'f1', 'auc', 'ece']}

    rng = np.random.default_rng(seed)
    n = labels.size

    metrics_samples = {'accuracy': [], 'precision': [], 'recall': [], 'f1': [], 'auc': [], 'ece': []}
    ...
    for _ in range(n_boot):
        ...
        # ECE for this bootstrap sample (multiclass)
        ece_val = _expected_calibration_error_multiclass(probs_s, labels_s)
        metrics_samples['ece'].append(float(ece_val))
