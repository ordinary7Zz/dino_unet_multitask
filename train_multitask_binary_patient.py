...
from patient_dataset import PatientMultiTaskDataset, collate_patient_bags
from dino_unet_multitask import DINOv3_S_UNet_MULTITASK
from utils.patient_metrics import (
    pool_instance_logits,
    evaluate_model_binary_target_patient,
    compute_patient_youden_threshold,
)
from utils.loss import structure_loss, benign_malignant_loss, benign_malignant_loss_gla
from utils.binary_stats import (
    summarize_binary_label_distribution_patient,
    gla_params_binary_patient,
    compute_binary_pos_weight_patient,
)
from utils.utils import log_print


parser = argparse.ArgumentParser('DINOV3-UNet patient-level binary multitask training')
...
parser.add_argument('--patient_id_depth', type=int, default=2,
                    help='number of path segments used to derive patient_id (default: 2)')
parser.add_argument('--max_images_per_patient', type=int, default=None,
                    help='maximum number of images sampled per patient during training only')
parser.add_argument('--cls_pooling', type=str, default='max', choices=['max', 'mean'],
                    help='pooling mode used to aggregate instance logits into patient logits')
parser.add_argument('--cls_loss', type=str, default='gla', choices=['gla', 'bce'],
                    help='classification loss type for patient-level training')
parser.add_argument('--cls_pos_weight', type=float, default=None,
                    help='positive class weight for BCE mode; if omitted in BCE mode, auto-compute at patient level')
parser.add_argument('--gla_tau', type=float, default=None,
                    help='GLA tau: 1.0=full adjustment, 0.0=no adjustment. If None, auto based on patient imbalance')
parser.add_argument('--fixed_threshold', type=float, default=0.5,
                    help='classification threshold used when val_threshold_mode=fixed')
parser.add_argument('--val_threshold_mode', type=str, default='fixed', choices=['fixed', 'youden'],
                    help='threshold selection mode for patient-level classification evaluation')
parser.add_argument('--threshold_dataset_name', type=str, default=None,
                    help='dataset name used to compute Youden threshold when val_threshold_mode=youden')
...
def _resolve_eval_threshold(model, dataset_names, datasets, device, log_file):
    if args.val_threshold_mode == 'fixed':
        threshold = float(args.fixed_threshold)
        log_print(f'Using fixed patient-level classification threshold: {threshold:.4f}\n', log_file)
        return threshold

    target_dataset_name = args.threshold_dataset_name or dataset_names[0]
    if target_dataset_name not in dataset_names:
        raise ValueError(
            f"threshold_dataset_name '{target_dataset_name}' was not found in test_dataset_names={dataset_names}"
        )

    dataset_index = dataset_names.index(target_dataset_name)
    threshold_dataset = datasets[dataset_index]
    ...
    threshold_info = compute_patient_youden_threshold(
        model,
        threshold_loader,
        device,
        cls_pooling=args.cls_pooling,
    )
    threshold = float(threshold_info.get('best_threshold', args.fixed_threshold))
    log_print(
        (
            f"Using patient-level Youden threshold from {target_dataset_name}: {threshold:.4f} "
            f"(youden={threshold_info.get('youden', float('nan')):.4f}, "
            f"sensitivity={threshold_info.get('sensitivity', float('nan')):.4f}, "
            f"specificity={threshold_info.get('specificity', float('nan')):.4f})\n"
        ),
        log_file,
    )
    return threshold
...
def _summarize_metrics(log_file, args, epoch_eval_points, epoch_dice_scores, epoch_hd95_scores, epoch_cls_metrics):
    log_print('\n========== Evaluation Summary (Patient-Level Classification) ==========\n\n', log_file)
    ...
    for dataset_name in args.test_dataset_names:
        ...
        if cls_list:
            ...
            youden = [m.get('youden', 0.0) for m in cls_list]

            log_print(f'\n{args.target_key} patient-level metrics per eval:\n', log_file)
            log_print(f'  Acc:  {acc}\n', log_file)
            ...
            log_print(f'  ECE:  {ece}\n', log_file)
            log_print(f'  Youden:{youden}\n', log_file)

            bi = _best_index(f1, mode='max')
            log_print(f'Best {args.target_key} patient-level F1: {f1[bi]:.4f} at epoch {eval_epochs[bi]}\n', log_file)
        else:
            log_print(f'\n{args.target_key} patient-level metrics per eval: []\n', log_file)
...
def main(args):
    ...
    save_dir = os.path.join('logs', args.dataset_name, f'{args.method}_{args.target_key}_patient_{timestamp}')
    os.makedirs(save_dir, exist_ok=True)

    log_name = os.path.join(save_dir, f'{args.method}_{args.dataset_name}_{args.target_key}_patient_log.log')
    log_file = open(log_name, 'w', encoding='utf-8')

    try:
        log_print(f'Training started at {time.ctime()}\n', log_file)
        log_print(f'Method: {args.method}\nDataset: {args.dataset_name}\nEpochs: {args.epoch}\n', log_file)
        log_print(f'Target Key: {args.target_key}\nLR: {args.lr}\nBatch Size (patients): {args.batch_size}\n', log_file)
        ...
        log_print(f'Patient ID depth: {args.patient_id_depth}\n', log_file)
        log_print(f'Max images per patient (train only): {args.max_images_per_patient}\n', log_file)
        log_print(f'Classification pooling: {args.cls_pooling}\n', log_file)
        log_print(f'Classification loss mode: {args.cls_loss}\n', log_file)
        log_print(f'Validation threshold mode: {args.val_threshold_mode}\n', log_file)

        dataset = PatientMultiTaskDataset(
        ...
        label_stats = dataset.get_patient_label_stats()
        bag_size_stats = dataset.get_bag_size_stats()
        log_print(
            (
                f"Training patient label stats for {args.target_key}: Negative={label_stats['negative']}, "
                f"Positive={label_stats['positive']}, Missing={label_stats['missing']}, Valid={label_stats['valid']}\n"
            ),
            log_file,
        )
        log_print(
            (
                f"Training patient bag size stats: min={bag_size_stats['min']}, "
                f"max={bag_size_stats['max']}, mean={bag_size_stats['mean']:.2f}\n"
            ),
            log_file,
        )
        ...
        log_print(
            (
                f"Using patient-level DataLoader shuffle=True, patient_count={len(dataset)}, "
        ...
        log_print(
            (
                f"First patient sample: patient_id={first_patient['patient_id']}, "
                f"num_images={first_patient['num_images']}, target={first_patient['target']}\n"
            ),
            log_file,
        )
        ...
        if args.cls_loss == 'gla':
            p_neg, p_pos, gla_tau = gla_params_binary_patient(
                args.train_label_path,
                args.target_key,
                patient_id_depth=args.patient_id_depth,
                gla_tau=args.gla_tau,
                log_file=log_file,
            )
            cls_pos_weight = None
        else:
            ...
            p_pos = None
            gla_tau = None
            if args.cls_pos_weight is None:
                cls_pos_weight = compute_binary_pos_weight_patient(
                    args.train_label_path,
                    args.target_key,
                    patient_id_depth=args.patient_id_depth,
                    log_file=log_file,
                )
                log_print(f'Auto-computed patient-level pos_weight: {cls_pos_weight:.6f}\n', log_file)
            else:
                cls_pos_weight = float(args.cls_pos_weight)
                log_print(f'Using manual patient-level pos_weight: {cls_pos_weight:.6f}\n', log_file)
        ...
        eval_datasets = []
        for dataset_name, img_path, mask_path, label_path in zip(
            args.test_dataset_names, args.test_image_paths, args.test_mask_paths, args.test_label_paths
        ):
            test_dataset = PatientMultiTaskDataset(
                img_path,
                mask_path,
                label_path,
                args.img_size,
            ...
            test_stats = test_dataset.get_patient_label_stats()
            test_bag_stats = test_dataset.get_bag_size_stats()
            log_print(
                (
                    f"Eval dataset {dataset_name} patient stats: Negative={test_stats['negative']}, "
                    f"Positive={test_stats['positive']}, Missing={test_stats['missing']}, Valid={test_stats['valid']}\n"
                ),
                log_file,
            )
            log_print(
                (
                    f"Eval dataset {dataset_name} bag size stats: min={test_bag_stats['min']}, "
                    f"max={test_bag_stats['max']}, mean={test_bag_stats['mean']:.2f}\n"
                ),
                log_file,
            )
        ...
        for epoch in range(args.epoch):
                ...
                for task_name in schedule:
                    ...
                    with torch.cuda.amp.autocast(enabled=bool(args.use_amp)):
                        ...
                        if task_name == 'seg':
                            ...
                            pooled_logits = pool_instance_logits(pred_cls, bag_sizes, pooling=args.cls_pooling)
                            valid = (targets != -1)
                            if valid.any():
                                valid_logits = pooled_logits[valid].unsqueeze(1)
                                valid_targets = targets[valid]
                                if args.cls_loss == 'gla':
                                    loss = cls_weight * benign_malignant_loss_gla(
                                        valid_logits,
                                        valid_targets,
                                        p_pos=p_pos,
                                        p_neg=p_neg,
            ...
            writer.add_scalar('TRAIN_CLS_PATIENT_LOSS', cls_loss_avg, epoch + 1)
            ...
            if (epoch + 1) % args.checkpoint_interval == 0 or epoch == args.epoch - 1:
                ckpt = os.path.join(checkpoint_path, f'{args.method}_{args.dataset_name}_{args.target_key}_patient_epoch_{epoch + 1}.pth')
            ...
            if (epoch + 1) % args.eval_interval == 0:
                ...
                model.eval()
                with torch.no_grad():
                    eval_threshold = _resolve_eval_threshold(model, args.test_dataset_names, eval_datasets, device, log_file)
                    for dataset_name, test_dataset in zip(args.test_dataset_names, eval_datasets):
                        ...
                        eval_results = evaluate_model_binary_target_patient(
                            model,
                            test_loader,
                            device,
                        ...
                        writer.add_scalar(f'VAL_CLS_YOUDEN/{dataset_name}', cls_metrics.get('youden', 0.0), epoch + 1)

                        log_print(f'  {dataset_name} - Dice: {dice:.4f}, HD95: {hd95:.4f}\n', log_file)
                        log_print(f'  {dataset_name} - {args.target_key} threshold: {eval_threshold:.4f}\n', log_file)
                        log_print(
                            f"  {dataset_name} - patient-level {args.target_key}: Acc={cls_metrics['accuracy']:.4f}, "
                            ...
                            f"Youden={cls_metrics['youden']:.4f}\n",
                            log_file
                        )
